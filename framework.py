...
class Framework:
    def __init__(self, config):
        ...
        if self._is_main_process() and getattr(config, "mode", "train") != "val":
            print("[Init] Baseline run")
            print(f"[Init] Checkpoint directory: {self.save_dir}")
            print(f"[Init] Log file: {self.log_path}")
    ...
    # Reduce model outputs to a scalar prediction per sample.
    ...
    # Apply per-atom scaling only when the loss is configured that way.
    ...
    def train(self, model, train_loader, val_loader, resume_state=None):
        ...
        if resume_state:
            ...
            if opt_state:
                try:
                    ...
                    self.logging(f"[Resume] Optimizer state has incompatible parameter groups: {exc}")
                else:
        ...
        for local_epoch in range(epochs_to_run):
            ...
            # Use the base temperature for this epoch unless a phase override is active.
            current_temperature = base_temperature
            # Keep the model reference in sync with the current epoch.
            base_model = model.module if hasattr(model, "module") else model
            ...
            while data is not None:
                ...
                # Optional edge-debug hook; left as a placeholder for local inspection.
                ...
                if self._is_main_process() and train_steps == 1:
                    ...
                    if s_self is not None and s_token is not None:
                        def _stat(t):
                        ...
                        self.logging(
                            f"[TIS Debug] S_self mean/min/max={self_stat[0]:.4f}/{self_stat[1]:.4f}/{self_stat[2]:.4f} | "
        ...
        self.config.temperature = current_temperature
        if self._is_main_process():
            # Persist the final resolved configuration alongside the checkpoint.
    ...
    def _flush_geom_samples(self, epoch):
        ...
        # Reset the per-epoch geometry sample buffer after flushing to disk.
    def _log_stoken_sort(self, model, mask, sid, tag, batch_idx=0, topk=10):
        ...
        if s_cpu.size(0) == 0:
            return
        b = 0  # Default to the first item when writing a representative log sample.
