...
# Core MADTP-style token selection utilities.
def vector_gather(vectors, indices):
    """
    Gather token vectors along the sequence dimension.

    Args:
        vectors: Tensor of shape [batch, seq_len, hidden_dim].
        indices: Token indices to gather for each batch item.

    Returns:
        Gathered vectors with the same batch size.
    """
    N, L, D = vectors.shape
    squeeze = False
    if indices.ndim == 1:
...
class Query_model(nn.Module):
    ...
    def __init__(self, ft_dim, sd_dim, temperature=1, att_func_type='softmax', pool_type='sum', map_func=True):
        ...
        # Optional projection from token space into dictionary space.
        if self.map_func:
    ...
    def forward(self, ft, sd, mask=None, return_token_att=False, temperature=1):
        """
        Args:
            ft: [batch, token_num, ft_dim] - token features.
            sd: [dict_size, sd_dim] - learned space dictionary.
            mask: [batch, token_num] - padding mask
            return_token_att: if True, also return raw token scores.
            temperature: softmax temperature.
        """
        # Move the dictionary to the same device as the batch.
        sd = sd.to(ft.device)

        # Map token features into dictionary space when requested.
        if self.map_func:
        ...
        # Scale by the dictionary dimension before applying softmax.
...
# Pruning block 1: query-based token importance estimation.
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, space_dict=None, temperature=0):
        ...
        token_scores = self.score_proj(x).squeeze(-1)  # [B, N]
        attn_weight_flat = F.softmax(token_scores, dim=-1)
        self.attention_map = attn_weight_flat
        self.token_attention = attn_weight_flat

        pooled_feature = torch.bmm(attn_weight_flat.unsqueeze(1), x).squeeze(1)  # [B, C]
        pooled_feature = self.out_proj(pooled_feature.unsqueeze(1)).squeeze(1)
        pooled_feature = self.layer_norm(pooled_feature)

        pruned_feature = None
        if temperature > 0 and space_dict is not None:
            pruned_feature = self.prune_proj(pooled_feature)  # [B, sd_dim]

        return pooled_feature, attn_weight_flat, pruned_feature

# Pruning block 2: projection head.
...
class CatGT(nn.Module):
    def __init__(self, config):
        ...
        # Energy head consumes the l=0 scalar token features produced by the Equiformer adapter.
        ...
        # Initialize the layer-id allowlists used by the freeze logic.
        ...
        self._geom_importance_logged_epoch = None
        self._current_epoch = None
        # Keep the geometry-distance aggregation stable by default.
    ...
    def forward(self, h, x, edges, strings, mask, target, pos_mask=None, temperature=0, return_feature=False, train=True):
        # Accept either a tuple or a Hugging Face BatchEncoding.
        ...
        # The original Q-Former aggregation path was removed.
        # Keep the per-atom token representation for downstream attention.

        # Capture text attentions only when explicitly requested.
        ...
        # Cross-attention uses MultiheadAttention; key_padding_mask expects True for padded tokens.
        ...
        s_self_pre = s_self
        s_token_pre = s_token
        if s_self_pre is None and s_token_pre is None:
            s_self_pre = geom_mask_float.clone()
            s_token_pre = geom_mask_float.clone()
        elif s_self_pre is None:
            s_self_pre = s_token_pre.clone()
        elif s_token_pre is None:
            s_token_pre = s_self_pre.clone()
        ...
        pooled_feature, attn_weight_flat, pruned_feature = self.attention_pool(llm_out, self.space_dict, temperature)
        ...
        if s_self is None and s_token is None:
            # Fall back to a uniform distribution over valid atoms if both branches are unavailable.
            fallback = geom_mask_bool.to(dtype=attn_weight_flat.dtype)
            s_self = fallback.clone()
            s_token = fallback.clone()
        elif s_self is None:
            s_self = s_token.clone()
        elif s_token is None:
            s_token = s_self.clone()
        s_self = s_self / (s_self.sum(dim=1, keepdim=True) + 1e-8)
        s_token = s_token / (s_token.sum(dim=1, keepdim=True) + 1e-8)
        # GTIS follows the paper: combine self-attention and semantic relevance only.
        tis = (s_self + s_token) / 2.0
        ...
        if return_feature:
            return adsorption_energy, pooled_feature, loss_pruning
