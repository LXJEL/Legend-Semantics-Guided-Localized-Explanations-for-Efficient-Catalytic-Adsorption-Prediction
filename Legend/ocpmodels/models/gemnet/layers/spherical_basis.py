...
class CircularBasisLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if cbf_name == "gaussian":
            self.cos_basis = GaussianSmearing(
            ...
            self.cos_basis = lambda cos: torch.stack(
                [f(cos) for f in sph_funcs], dim=1
            )
        else:
            raise ValueError(f"Unknown cosine basis function '{cbf_name}'.")

    def forward(self, D_ca, cos_cab, id3_ca):
        rbf = self.radial_basis(D_ca)  # (num_edges, num_radial)
        cbf = self.cos_basis(cos_cab)  # (num_triplets, num_spherical)
