...
class BaseTrainer(ABC):
    ...
    def save(
        ...
        if not self.is_debug and distutils.is_master():
            def _dump_config_snapshot():
                # Keep a human-readable copy of the config next to the checkpoint.
                config_filename = (
                    os.path.splitext(checkpoint_file)[0] + "_config.yml"
                )
                config_path = os.path.join(
                    self.config["cmd"]["checkpoint_dir"], config_filename
                )
    ...
    def validate(self, split="val", disable_tqdm=False):
        ...
        if not disable_tqdm and distutils.is_master():
            iter_loader = tqdm(
                iter_loader,
                total=len(loader),
                position=rank,
                desc=f"{split} ({len(loader)} iters)",
                leave=False,
            )
