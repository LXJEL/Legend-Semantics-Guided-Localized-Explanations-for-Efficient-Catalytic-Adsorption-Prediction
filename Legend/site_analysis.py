...
class SiteAnalyzer:
    ...
    def _get_center_ads_atom(self):
        ...
        # If adsorbate_atom_idxs is empty, there is no tag==2 atom and thus no adsorbate.
        adsorbate_atom_idxs = [idx for idx, tag in enumerate(tags) if tag == 2]
        if adsorbate_atom_idxs == None:
    ...
    def _find_binding_graph(self):
        ...
        for idx in adsorbate_atom_idxs:
            if sum(connectivity[idx][slab_atom_idxs]) >= 1:
                bound_slab_idxs = [ idx_slab for idx_slab in slab_atom_idxs if connectivity[idx][idx_slab] == 1 ]
                ads_idx_info = {
                    "adsorbate_idx": idx,
