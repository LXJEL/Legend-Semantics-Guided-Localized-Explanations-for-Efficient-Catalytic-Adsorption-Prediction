...
def get_args():
    ...
    parser.add_argument('--dataset', type=str,  # Backward compatibility for older scripts.
    ...
    parser.add_argument('--use_catberta_llrd', dest='use_catberta_llrd', action='store_true',
                        ...
                        help='Use the CatBERTa-style layer-wise learning-rate decay schedule.')
    ...
    parser.add_argument('--llm_lr_base', type=float, default=2e-5,
                        ...
                        help='Base learning rate for the language-model backbone when LLRD is enabled.')
    ...
    parser.add_argument('--finetune_plan', type=str, default=None,
                        ...
                        help='Stage-2 fine-tuning plan in YAML/JSON format, e.g. "epochs,llm_layers,text_layers,lr_scale;...".')
...
def main():
    ...
    setattr(args, "dataset_root", ds_root)  # Resolve a single dataset root for downstream code.
    ...
    if is_distributed:
        ...
        if not torch.cuda.is_available():
            ...
            raise RuntimeError("CUDA is required for distributed training, but torch.cuda.is_available() is False.")
    ...
    # Print a concise run header once on the main process.
