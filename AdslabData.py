...
class AdslabData(Dataset):
    ...
    def __init__(
        ...
        # Load metadata once to capture length and label statistics, then drop references.
        temp_data = torch.load(path)
        if len(temp_data) < 7:
            raise ValueError(f"{path} contains {len(temp_data)} fields, expected at least 7.")
        ...
        if len(set(lengths)) != 1:
            raise ValueError(f"Inconsistent field lengths in {path}: {lengths}")
